// taken from https://github.com/ecnerwala/cp-book/blob/master/src/tensor.hpp
#include <array>
#include <cstdint>
...
template <typename T>
tensor <T, 1> v1d_to_tensor(std::vector <T>& v) {
	tensor <T, 1> res(std::array{v.size()});
...
template <typename T>
tensor <T, 2> v2d_to_tensor(std::vector <std::vector <T>>& v) {
	if (v.empty()) {
		return tensor <T, 2>(std::array{0, 0});
	}
	tensor <T, 2> res(std::array{v.size(), v[0].size()});
	for (std::size_t i = 0; i < v.size(); ++i) {
		for (std::size_t j = 0; j < v[0].size(); ++j) {
			res[i][j] = v[i][j];
		}
	}
	return res;
